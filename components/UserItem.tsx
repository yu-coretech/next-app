...
export const UserItem: VFC<Props> = memo(({
    ...
    <div className='my-1'>
        ...
        <button className='mr-1 py-1 px-3 text-white bg-green-600 hover:bg-green-700 rounded-2xl focus:outline-none'
        data-testid={`edit-${user.id}`} onClick={() => {setEditedUser(user)}}>Edit</button>
        <button className='mr-1 py-1 px-3 text-white bg-green-600 hover:bg-green-700 rounded-2xl focus:outline-none'
        data-testid={`delete-${user.id}`} onClick={async () => { await delete_users_by_pk({
            variables: {
                id: user.id,
            }
        })}}>Delete</button>
